...
class OptimizationTracker:
    ...
    def set_planner(self, planner):
        """Set current planner and reset progress."""
...
class TreePlanner:
    def __init__(self, width, length, tree_distance, randomness=0.3, method="perlin"):
        ...
        # placement method: 'perlin' or 'poisson'
        self.method = method
        self.trees = []
    ...
    def generate_tree_positions(self, callback=None):
        ...
        logger.info(
            "Starting tree placement optimization using %s method "
            "for %sx%sm area with %sm spacing",
            self.method,
            self.width,
            self.length,
            self.tree_distance,
        ...
        print(
            f"🌲 Starting {self.method} optimization for "
    ...
    def _generate_perlin_positions(self, callback=None):
        ...
        best_trees = []
        best_count = 0
        max_iterations = MAX_PERLIN_ITERATIONS  # Try multiple random patterns

        for iteration in range(max_iterations):
            logger.info(
                "Perlin iteration %d/%d - Testing new random pattern",
                iteration + 1,
                max_iterations,
            )
            print(
                f"🔄 Perlin Iteration {iteration + 1}/{max_iterations} - "
                f"Testing new pattern..."
            )
            self.trees = []
            ...
            for row in range(rows):
                for col in range(cols):
                    ...
                    if self.randomness > RANDOMNESS_THRESHOLD:
                        displaced_x += (
                            (random.random() - 0.5) * self.tree_distance * 0.3
                        )
                        displaced_y += (
                            (random.random() - 0.5) * self.tree_distance * 0.3
                        )
            ...
            logger.info(
                "Perlin iteration %d completed: %d trees placed",
                iteration + 1,
                current_count,
            )
            ...
            if current_count > best_count:
                ...
                logger.info(
                    "New best result found: %d trees (improvement from previous best)",
                    best_count,
                )
        ...
        logger.info(
            "Perlin optimization completed: Final result has %d trees", len(self.trees)
        )
    ...
    def _generate_poisson_positions(self, callback=None):
        ...
        For natural distribution pattern.
        """
        best_trees = []
        best_count = 0
        # Fewer iterations for Poisson as each is more expensive
        max_iterations = MAX_POISSON_ITERATIONS

        for iteration in range(max_iterations):
            logger.info(
                "Poisson iteration %d/%d - Generating new distribution",
                iteration + 1,
                max_iterations,
            )
            print(
                f"🎯 Poisson Iteration {iteration + 1}/{max_iterations} - "
                f"Generating distribution..."
            )
            ...
            current_count = len(self.trees)
            logger.info(
                "Poisson iteration %d completed: %d trees placed",
                iteration + 1,
                current_count,
            )
        ...
        self.trees = best_trees
        logger.info(
            "Poisson optimization completed: Final result has %d trees", len(self.trees)
        )
    ...
    def _poisson_disc_sampling(self):
        ...
        # Number of attempts to place around each active point
    ...
    def _is_valid_position(self, new_tree):
        """Check if a new tree position maintains minimum distance.
    ...
    def generate_planting_image(self):
        ...
        if self.method == "perlin":
            ...
            subtitle += (
                f"Randomness: {self.randomness:.1f} (Colors show displacement forces)"
            )
        else:  # poisson
            ...
            subtitle += (
                f"Randomness: {self.randomness:.1f} (Natural distribution pattern)"
            )
...
def generate_plan():
    try:
        ...
        def progress_callback(iteration, count, max_iterations, is_best):
            ...
            tracker.progress_log.append(progress_data)
        ...
        return (
            jsonify({"error": "Invalid input values. Please enter valid numbers."}),
            400,
        )
...
def generate_plan_stream():
    ...
    try:
        ...
        # Validate inputs - combine both validation checks
        error_msg = None
        if width <= 0 or length <= 0 or tree_distance <= 0:
...
def download_coordinates():
    try:
        ...
        filename = (
            f"tree_coordinates_{data['area']['width']}x{data['area']['length']}.json"
        )
        ...
        return jsonify({"error": f"Download failed: {e!s}"}), 500
